...
%                 p = p_corr0  - c.*sate_rel_corr - sate_vel_rad.*del_t + T + PC_bias + c*rece_corr ;
%                 lhs = PC- p;
%             case 'biases'
%                 p = p_corr0 + PC_bias;
%                 lhs = GARM_C1(i,:) - p;
%             case 'No biases'
%             % No bias       
%                 p = p_corr0  - c.*sate_rel_corr - sate_vel_rad.*del_t + T + c*rece_corr;
%                 lhs = PC- p;
%             case 'No ion'
%                 p = p_corr0  - c.*sate_rel_corr - sate_vel_rad.*del_t + T + bias + c*rece_corr;
%                 lhs = GARM_C1(i,:) - p;    
%             case 'No tropo'
%              % without tropospheric correction 
%                 p = p_corr0  - c.*sate_rel_corr - sate_vel_rad.*del_t + PC_bias + c*rece_corr;
%                 lhs = PC- p;
%             case 'No rel'
%              % without relativistic correction
%                 p = p_corr0  - c.*Sat_clk(i,:) - sate_vel_rad.*del_t + T + PC_bias + c*rece_corr;
%                 lhs = PC- p;
%             case 'No rot'
                % without sagnac correction
%                 p = p_raw  - c.*sate_rel_corr - sate_vel_rad.*del_t + T + PC_bias + c*rece_corr;
%                 lhs = PC- p;
%             case 'No light'
%                 % without light travel correction 
%                 p = p_corr0  - c.*sate_rel_corr + T + PC_bias + c*rece_corr;
%                 lhs = PC- p;
%             case 'No clock'
%                 % without satellite clock correction 
%                 p = p_corr0  - sate_vel_rad.*del_t + T + PC_bias + c*rece_corr;
%                 lhs = PC- p;
%         end
%         

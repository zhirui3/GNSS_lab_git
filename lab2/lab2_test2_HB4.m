...
%a priori coords used at end, don't delete
...
C1 = load('GARM_C1.obs');       % C1 observation
P2 = load('GARM_P2.obs');       % P2 observation
coor = load('GARM.crd');         % station coordinates 
 
end 

if station=='k'
    
C1 = load('KIT_C1.obs');       % C1 observation
P2 = load('KIT_P2.obs');       % P2 observation
coor = load('KIT.crd');         % station coordinates 
...
for i = 1:n 
    ...
    while  (1)
        ...
        % 6. Ionospheric delay 
        PC = (f1^2*C1(i,:) - f2^2*P2(i,:))./(f1^2 - f2^2);
...
if station=='k'
    del_p_k=del_p_mat;
    sigma_p_k=sigma_p_mat;
    pos_k_x=pos_x;
    pos_k_y=pos_y;
    pos_k_z=pos_z;
    
    save('KIT_file.mat', 'pos_k_x', 'pos_k_y','pos_k_z', 'sigma_p_k', 'del_p_k');
    
end

if station=='g'
    del_p_g=del_p_mat;
    sigma_p_g=sigma_p_mat;
    pos_g_x=pos_x;
    pos_g_y=pos_y;
    pos_g_z=pos_z;
    
    
   save('GARM_file.mat', 'pos_g_x', 'pos_g_y','pos_g_z', 'sigma_p_g', 'del_p_g');
end

load('KIT_file.mat', 'pos_k_x')
load('KIT_file.mat', 'pos_k_y')
load('KIT_file.mat', 'pos_k_z')

load('GARM_file.mat', 'pos_g_x')
load('GARM_file.mat', 'pos_g_y')
load('GARM_file.mat', 'pos_g_z')
...
delx=pos_g_x-pos_k_x;
dely=pos_g_y-pos_k_y;
delz=pos_g_z-pos_k_z;
...
figure(4) 
plot(epoch./3600 ,delx,'.');
hold on 
plot(epoch./3600 ,dely,'.');
plot(epoch./3600 ,delz,'.');
plot(epoch./3600 ,delx_ap*ones(size(epoch./3600)));
plot(epoch./3600 ,dely_ap*ones(size(epoch./3600)));
plot(epoch./3600 ,delz_ap*ones(size(epoch./3600)));

xlabel('Epoch [hours]')
ylabel('Difference [m]')
legend('N','E','U', 'N (A Priori)','E (A Priori)','U (A Priori)')
title('GARM-KIT Difference','FontSize',10)

% figure(5) 
% plot(epoch./3600 ,delx_ap,'-');
% hold on 
% plot(epoch./3600 ,dely_ap,'.');
% plot(epoch./3600 ,delz_ap,'.');
% xlabel('Epoch [hours]')
% ylabel('Difference [m]')
% legend('N','E','U')
% title('GARM-KIT A Priori Difference','FontSize',10)
